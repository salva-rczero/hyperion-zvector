...
/* While searching for a better solution, each time a VR is read or  */
/* written, the contents of the FPR part are refreshed.              */
/*-------------------------------------------------------------------*/
#define REFRESH_READ_VR(_vr)                                          \
    do {                                                              \
        if ((_vr) < 16)                                               \
        {                                                             \
            regs->vr[(_vr)].F[0] = CSWAP32(regs->fpr[FPR2I((_vr))]);  \
            regs->vr[(_vr)].F[1] = CSWAP32(regs->fpr[FPR2I((_vr))+1]);\
        }                                                             \
    } while(0)
...
#define REFRESH_UPDATE_VR(_vr)                                        \
    do {                                                              \
        if ((_vr) < 16)                                               \
        {                                                             \
            regs->fpr[FPR2I((_vr))] = CSWAP32(regs->vr[(_vr)].F[0]);  \
            regs->fpr[FPR2I((_vr))+1] = CSWAP32(regs->vr[(_vr)].F[1]);\
        }                                                             \
    } while(0)
